...
<head>
  ...
  <title> STI Registrar | Edit Section </title>
  <!-- plugins:css -->
              ...
              <div class="home-tab">
                ...
                <div class="tab-content tab-content-basic">
                  <div class="tab-pane fade show active" id="overview" role="tabpanel" aria-labelledby="overview"> 
                    ...
                    <?php 
                      $section_id = $_GET['editSection'];
                      $getSection = mysqli_query($db, "SELECT * FROM tbl_sections WHERE section_id = '$section_id'");
                    ?>
                    ...
                    <div class="col-lg-12 grid-margin stretch-card">
                      <div class="card">
                        <div class="card-body">
                          ...
                          <form action="maintenanceSection_edit.php" method="POST">
                            <input type="hidden" value="<?php echo $section_id; ?>" name="id">
                            <?php while($srow = mysqli_fetch_array($getSection)) { ?>
                            <div class="form-group row">
                              <div class="col">
                                <div class="form-group">
                                    <label>Course</label>
                                    <select class="js-example-basic-single w-100 select2-hidden-accessible" data-select2-id="1" tabindex="-1" aria-hidden="true" style="padding: 10px;" name="course">
                                      <?php $courseResults = mysqli_query($db, "SELECT course_code FROM tbl_courses");?>
                                      <?php while($row = mysqli_fetch_array($courseResults)) { ?>
                                        <option value="<?php echo $row['course_code']; ?>" <?php if($row['course_code'] == $srow['course']) echo 'selected="selected"'?>><?php echo $row['course_code']; ?></option>
                                      <?php } ?>
                                    </select>
                                  </div>
                              </div>
                              <div class="col">
                                <p>Section Number:</p>
                                <input class="form-control form-control-lg" type="number" name="sectionNumber" placeholder="Section Number" value="<?php echo $srow['section_num']; ?>" required="">
                              </div>
                            </div>

                            <div class="form-group row">
                              <div class="col">
                                <button class="btn btn-primary btn-lg" name="editSection" style="color: white; width: 100%; margin-bottom: 0;"> Update Section </button>
                              </div>
                            </div>
                            <?php } ?>
                          </form>
