...
<head>
  ...
  <title> STI Registrar | Edit Subject </title>
  <!-- plugins:css -->
              ...
              <div class="home-tab">
                ...
                <div class="tab-content tab-content-basic">
                  <div class="tab-pane fade show active" id="overview" role="tabpanel" aria-labelledby="overview"> 
                    ...
                    <?php 
                      $subject_id = $_GET['editSubject'];
                      $subject_idA = $_GET['editSubject'];
                      $getSubject = mysqli_query($db, "SELECT * FROM tbl_subjects WHERE subject_id = '$subject_id'");
                      $getSubjectA = mysqli_query($db, "SELECT section FROM tbl_subjects WHERE subject_id = '$subject_idA'");
                    ...
                    <div class="col-lg-12 grid-margin stretch-card">
                      <div class="card">
                        <div class="card-body">
                          ...
                          <form action="maintenanceSubject_edit.php" method="POST">
                            <input type="hidden" value="<?php echo $subject_id; ?>" name="id">
                            
                            <?php while($subrow = mysqli_fetch_array($getSubject)) { ?>
                            <div class="form-group">
                              ...
                              <select class="js-example-basic-single w-100 select2-hidden-accessible" data-select2-id="1" tabindex="-1" aria-hidden="true" style="padding: 10px;" name="course" id="course" required>
                                ...
                                <?php while($row1 = mysqli_fetch_array($courseResults)) { ?>
                                  <option value="<?php echo $row1['course_code']; ?>" 
                                    <?php 

                                    $explodeCourse = explode(",", $subrow['section']);
                                    $getFirstValue = array_shift($explodeCourse);
                                    $course = preg_replace('/[^a-zA-Z]/', '', $getFirstValue);

                                    if($row1['course_code'] == $course) echo 'selected="selected"'

                                    ?>

                                  ><?php echo $row1['course_description']; ?></option>
                                <?php } ?>
                              </select>
                            </div>
                            <div class="form-group row">
                              <div class="col">
                                <div class="form-group">
                                    ...
                                    <select class="js-example-basic-single w-100 select2-hidden-accessible" data-select2-id="1" tabindex="-1" aria-hidden="true" style="padding: 10px;" name="section[]" id="section" multiple>

                                      <!-- Edit Start Here -->

                                      <?php 

                                      $subrowA = mysqli_fetch_array($getSubjectA);
                                      $explodeCourseA = explode(",", trim($subrowA['section']));
                                      $explodeCourseA = array_map('trim',$explodeCourseA);
                                      $getFirstValueA = explode(",", trim($subrowA['section']))[0];
                                      $courseA = preg_replace('/[^a-zA-Z]/', '', $getFirstValueA);
                                      $sectionResults = mysqli_query($db, "SELECT section FROM tbl_sections WHERE course = '$courseA'");
                                      $result_array = mysqli_fetch_all($sectionResults, MYSQLI_ASSOC);
                                      echo '<script>alert("'.print_r($explodeCourseA, true).'")</script>';
                                      echo var_dump($subrowA['section']);
                                      foreach($result_array as $row){
                                      ?>
                                      <option value="<?php echo $row['section'];?>"
                                          <?php if (in_array(trim($row['section']), array_map('trim', $explodeCourseA))) { echo "selected"; } ?>
                                      >
                              ...
                              <div class="col">
                                <p>Subject Code:</p>
                                <input class="form-control form-control-lg" type="text" name="subjectCode" placeholder="Subject Code" value="<?php echo $subrow['subject_code']; ?>" required="">
                              </div>
                              <div class="col">
                                <p>Subject Description:</p>
                                <input class="form-control form-control-lg" type="text" name="subjectDescription" placeholder="Subject Description" value="<?php echo $subrow['subject_description']; ?>" required="">
                              </div>
                            </div>

                            <div class="form-group row">
                              <div class="col">
                                <button class="btn btn-primary btn-lg" name="editSubject" style="color: white; width: 100%; margin-bottom: 0;"> Update Subject </button>
                              </div>
                            </div>
                            <?php } ?>
                          </form>
        ...
        <script type="text/javascript">
          $(document).ready(function(){
            $('#course').on('change', function(){
                var courseValuesSubEd = this.value;
                if(courseValuesSubEd){
                    $.ajax({
                        type:'POST',
                        url:'getDropdownValues.php',
                        data:'courseValuesSubEd='+courseValuesSubEd,
