...
<head>
  ...
  <title> STI Registrar | Edit Subject Proctors </title>
  <!-- plugins:css -->
                ...
                <div class="tab-content tab-content-basic">
                  <div class="tab-pane fade show active" id="overview" role="tabpanel" aria-labelledby="overview"> 
                    ...
                    <div class="col-lg-12 grid-margin stretch-card">
                      <div class="card">
                        <div class="card-body">
                          <h4 class="card-title">Edit Subject Proctors</h4>
                          <?php include('successPrompt.php');?>

                          <form action="permitManager_edit.php" method="POST">
                            <div class="form-group">
                              ...
                              <select class="js-example-basic-single w-100 select2-hidden-accessible" data-select2-id="1" tabindex="-1" aria-hidden="true" style="padding: 10px;" name="section" id="section" required>
                                ...
                                <?php 
                                  $db = mysqli_connect('localhost','root','','db_sepc') or die("Unable to connect to database");
                                  $sectionsResults = mysqli_query($db, "SELECT section FROM tbl_sections");
                                ?>
                            ...
                            <div class="form-group row">
                              <div class="col">
                                <div class="form-group">
                                  <label>Subject</label>
                                  <select class="js-example-basic-single w-100 select2-hidden-accessible" data-select2-id="1" tabindex="-1" aria-hidden="true" style="padding: 10px;" name="subject" id="subject" required>
                                  </select>
                                </div>
                              </div>
                              <div class="col">
                                <div class="form-group">
                                  ...
                                  <select class="js-example-basic-single w-100 select2-hidden-accessible" data-select2-id="1" tabindex="-1" aria-hidden="true" style="padding: 10px;" name="quarter">
                                    <?php $quarterResult = mysqli_query($db, "SELECT quarter FROM tbl_qs");?>
                                    <?php while($qrow = mysqli_fetch_array($quarterResult)) { ?>
                                      <option value="Prelim" <?php if($qrow['quarter'] == "Prelim") echo 'selected="selected"'?>>Prelim</option>
                                      <option value="Midterm" <?php if($qrow['quarter'] == "Midterm") echo 'selected="selected"'?>>Midterm</option>
                                      <option value="Prefinal" <?php if($qrow['quarter'] == "Prefinal") echo 'selected="selected"'?>>Prefinal</option>
                                      <option value="Final" <?php if($qrow['quarter'] == "Final") echo 'selected="selected"'?>>Final</option>
                                    <?php } ?>
                                  </select>
                                </div>
                              </div>
                              <div class="col">
                                <div class="form-group">
                                  ...
                                  <select class="js-example-basic-single w-100 select2-hidden-accessible" data-select2-id="1" tabindex="-1" aria-hidden="true" style="padding: 10px;" name="proctor">
                                    <?php $proctorResult = mysqli_query($db, "SELECT initials FROM tbl_proctors");?>
                                    <?php while($prow = mysqli_fetch_array($proctorResult)) { ?>
                                      <option value="<?php echo $prow['initials']; ?>"><?php echo $prow['initials']; ?></option>
                                    <?php } ?>
                                  </select>
                                </div>
                              </div>
                            ...
                            <div class="form-group row">
                              <div class="col">
                                <button class="btn btn-primary btn-lg" name="editSubProctor" style="color: white; width: 100%; margin-bottom: 0;"> Update Subject Proctor </button>
                              </div>
                            </div>
                          </form>
                        </div>
        ...
        <script type="text/javascript">
          $(document).ready(function(){
            $('#section').on('change', function(){
                var sectionsValues = this.value;

                if(sectionsValues){
                    $.ajax({
                        type:'POST',
                        url:'getDropdownValues.php',
                        data:'sectionsValues='+sectionsValues,
            ...
            var defaultOption = $('#section').find('option[value=""]');
