...
<head>
  ...
  <title> STI Registrar | Promissory Manager </title>
  <!-- plugins:css -->
                ...
                <div class="tab-content tab-content-basic">
                  <div class="tab-pane fade show active" id="overview" role="tabpanel" aria-labelledby="overview"> 
                    ...
                    if (isset($_POST['searchSubmitted'])) {
                      ...
                      $query = "SELECT * FROM tbl_promissory WHERE CONCAT(student_id) LIKE '%".$find_value."%'";
                      $searchResult = filterTable($query);

                    } else {

                      $query = "SELECT * FROM tbl_promissory";
                      $searchResult = filterTable($query);
                    ...
                    <form action="promissoryManager.php" method="POST">
                      <div class="row">
                        <div class="col-lg-12 d-flex flex-column">
                          <div class="card">
                            <div class="card-body">
                              <div class="row">
                                <div class="col-md-6">
                                  <h4 class="card-title">Manage Promissory Notes</h4>
                                </div>
                                <div class="col-md-6">
                                  <div class="row">
                                    <div class="col-sm-2">
                                      <button type="submit" class="btn btn-inverse-dark btn-icon" name="searchSubmitted">
                                        <i class="mdi mdi-magnify"></i>
                                      </button>
                                    </div>
                                    <div class="col-sm-10">
                                      <input type="text" class="form-control form-control-md" placeholder="Search Student ID" aria-label="Search" style="float: right; margin-top: 2px;" name="find_value">
                                    </div>
                                  </div>
                                </div>
                              </div>
                              <p class="card-description">
                                Viewing all of data inside the database of Promissory. You are able to Edit and Delete or basically modify promissory within the Database of Promissory.
                              </p>
                                  ...
                                  <tbody>
                                    <?php while($row = mysqli_fetch_array($searchResult)) { ?>
                                      <tr>
                                        ...
                                        <td>
                                          <button type="button" class="btn btn-primary" onclick="approvePromissory('<?php echo $row['student_id']; ?>', '<?php echo $row['student_id']; ?>')" style="color: white;">Approve</button></td>
                                      </tr>
                                    <?php } ?>
                                  </tbody>

                                  <script type="text/javascript">
                                    function approvePromissory(approveID, studID){
                                      if (confirm("Are you sure you want to approve promissory from - " + studID + "?")) {
                                          window.location.href='deleteProcess.php?approvePromissory='+ approveID;
                                      }
                                    }
                                  </script>
                    ...
                    if (isset($_POST['searchApprove'])) {
                      
                      $find_value = $_POST['find_value'];
                      $query = "SELECT * FROM tbl_promissorya WHERE CONCAT(student_id) LIKE '%".$find_value."%'";
                      $searchResultA = filterTableApproved($query);

                    } else {

                      $query = "SELECT * FROM tbl_promissorya";
                      $searchResultA = filterTableApproved($query);

                    }

                    function filterTableApproved($query){
                      $db = mysqli_connect('localhost','root','','db_sepc') or die("Unable to connect to database");
                      $filterResult = mysqli_query($db, $query);
                      return $filterResult;
                    }
                        ...
                        <div class="col-lg-12 d-flex flex-column">
                          <div class="card">
                            <div class="card-body">
                              <div class="row">
                                <div class="col-md-6">
                                  <h4 class="card-title">Approved Promissory Notes</h4>
                                </div>
                                <div class="col-md-6">
                                  <div class="row">
                                    <div class="col-sm-2">
                                      <button type="submit" class="btn btn-inverse-dark btn-icon" name="searchApprove">
                                        <i class="mdi mdi-magnify"></i>
                                      </button>
                                    </div>
                                    <div class="col-sm-10">
                              ...
                              <div class="table-responsive">
                                <table class="table table-striped">
                                  ...
                                  <tbody>
                                    <?php while($row = mysqli_fetch_array($searchResultA)) { ?>
                                      <tr>
                                        <td><?php echo $row['promissory_id']; ?></td>
                                        <td><?php echo $row['student_id']; ?></td>
                                        <td><?php echo $row['promissory_note']; ?></td>
                                        ...
                                        <td><?php echo $row['quarter']; ?></td>
                                        <td>
                                          <button type="button" class="btn btn-primary" onclick="completePromisory('<?php echo $row['student_id']; ?>')" style="color: white;">Payment Completed</button></td>
                                      </tr>
                                    <?php } ?>
                                  </tbody>

                                  <script type="text/javascript">
                                    function completePromisory(studID){
                                      if (confirm("Are you sure you want to mark promisory completed from - " + studID + "?")) {
                                          window.location.href='deleteProcess.php?completePromisory='+ studID;
                                      }
                                    }
                                  </script>
                                </table>
